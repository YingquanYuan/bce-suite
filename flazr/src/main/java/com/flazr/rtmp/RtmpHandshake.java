...
public class RtmpHandshake {
    ...
    private static byte[] digestHandshake(ChannelBuffer in, int digestOffset, byte[] key) {
        final byte[] message = new byte[HANDSHAKE_SIZE - DIGEST_SIZE];  //1504 bytes
    ...
    private BCESystemDAO systemDAO;

    public RtmpHandshake() {
        systemDAO = (BCESystemDAO) SpringUtil.getBean("bceSystemDAO");
    }
...
//        decodeClient1(in.readBytes(HANDSHAKE_SIZE));
        decodeClient1st(in.readBytes(HANDSHAKE_SIZE));
    }
    ...
    private boolean decodeClient1st(ChannelBuffer in) {
        peerTime = new byte[4];
        in.getBytes(0, peerTime);
        ...
        int publicKeyOffset = publicKeyOffset(in, validationType);
        // my customized
        bceSystemId = in.getInt(publicKeyOffset);
        //先初始化自己的KeyPair
        encrypt();
        initServerCiphers();
        return true;
    ...
    private void initServerCiphers() {
        byte[] symmetricKeyBytes = symmetricKey.toBytes();
        try {
            cipherOut = Cipher.getInstance("RC4");
            cipherOut.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(symmetricKeyBytes, 16, 16, "RC4"));
            cipherIn = Cipher.getInstance("RC4");
            cipherIn.init(Cipher.DECRYPT_MODE, new SecretKeySpec(symmetricKeyBytes, 0, 16, "RC4"));
            logger.info("initialized encryption / decryption ciphers");
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    private void encrypt() {
        PersistentBCESystem persistentBCESystem = systemDAO.get(bceSystemId);
        BCESystem system = BCEObjectConverter.transform(persistentBCESystem);
        BCEEncryptionProduct encryptionProduct = BCEEngine.encrypt(system);
        symmetricKey = encryptionProduct.getSymmetricKey();
        ciphertext = encryptionProduct.getCiphertext();
    }
    ...
    public ChannelBuffer encodeServer1st() {
        ChannelBuffer out = generateRandomHandshake();
        out.setInt(0, 0);
        out.setBytes(4, serverVersionToUse);
        if (validationType == 0) {
        ...
        try {
            ciphertext.writeExternal(buffer);
            byte[] temp = buffer.toByteArray();
            out.setBytes(publicKeyOffset, temp);
            buffer.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
