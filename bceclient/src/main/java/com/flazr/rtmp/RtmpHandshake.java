...
public class RtmpHandshake {
    ...
    public RtmpHandshake(ClientOptions session) {
        ...
        this.bceSystemId = session.getBceSystemId();
        this.privateKeyFileName = session.getPrivateKeyFileName();
        this.paramsFileName = session.getParamsFileName();
    }
    ...
    private void initClientCiphers() {
    	...
    	try {
            ...
            cipherIn.init(Cipher.DECRYPT_MODE, new SecretKeySpec(symmetricKeyBytes, 16, 16, "RC4"));
...
//        decodeServer1(in.readBytes(HANDSHAKE_SIZE));
        decodeServer1st(in.readBytes(HANDSHAKE_SIZE));
    ...
    private BCEClientSystem system;
    private String paramsFileName;
    private BCEPrivateKey privateKey;
    private String privateKeyFileName;
    private int bceSystemId;
    private void decodeServer1st(ChannelBuffer in) {
        peerTime = new byte[4];
        in.getBytes(0, peerTime);
        byte[] serverVersion = new byte[4];
        in.getBytes(4, serverVersion);
        ...
        logger.info("server part 1 validation success");
        bceCiphertextBytes = new byte[208];
        int publicKeyOffset = publicKeyOffset(in, validationType);
        in.getBytes(publicKeyOffset, bceCiphertextBytes);
