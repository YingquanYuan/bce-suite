...
function checkEmail(id) {
	...
	if (str == null || str == "") {
		...
		$("#email_error").html(img).append("\<span\>Please input Email!\<\/span\>");
		return;
	}
	...
	if (!reg.test(str)) {
		...
		$("#email_error").html(img).append("\<span\>Please input correct Email!\<\/span\>");
		return;
	}
	$.ajax({
		type : "post",
		...
		success : function(json) {
			...
			if (json.result.toString().trim() == "success")
				...
				$("#email_error").html(img).append("\<span\>This email has been used!\<\/span\>");
		}
	});
}
...
function checkPwdQty(id) {
	...
	if (pwd.length < 5) {
		...
		$("#pwd_error").html(img).append("\<span\>Password too short, input a longer one!\<\/span\>");
		return;
	}
	if (pwd.match(/[a-z]/ig))
		level++;
...
function checkPwdAccd(prev, cur) {
	...
	if (prevPwd != curPwd) {
		...
		$("#pwd2_error").html(img)
				.append("\<span\>Different password, please correct it!\<\/span\>");
		return;
	}
	var img = "<img alt=\"Qualified password!\" src=\"images\/pic_success.gif\" style=\"vertical-align:middle\" />";
	$("#pwd2_error").html(img);
