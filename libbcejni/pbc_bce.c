...
void StoreGlobalParams(char *systemFileName, global_broadcast_params_t gbp)
{
    ...
    FILE *curveFile = fopen(gbp->pairFileName, "r");
    if (!curveFile)
    {
        printf("ACK! couldn't read curve file d201.  I die\n");
        return;
    }
                ...
                broadcast_system_t *sys)
{
    ...
    char param_string[16384];
    size_t count = fread(param_string, 1, 16384, params);
                              ...
                              ct_t myct)
{
    ...
    element_pairing(temp, myct->C1, mykey->h_i);
    //G1 element in denom
...
/** 输入：全局广播参数， 输出：广播系统 */
void Gen_broadcast_system(global_broadcast_params_t gbp, broadcast_system_t *sys)
{
    if(!gbp)
    {
        printf("ACK!  You gave me no broadcast params!  I die.\n");
                                   ...
                                   int num_users, char *pairFileName)
{
    ...
    if (pairing_init_set_buf(gbs->pairing, param_string, count))
        pbc_die("pairing init failed"); //Reports error message and exits with code 128.
    ...
    lhs = pbc_malloc(2 * num_users * sizeof(element_t));

    /*
    printf("DEBUG INFO: \n");
    printf("num_users = %d, 2 * num_users = %d\n", gbs->num_users, 2 * num_users);
    printf("size of element_t = %d\n", sizeof(element_t));
    printf("size of lgs = %d\n", sizeof(lgs));
    printf("size of lhs = %d\n", sizeof(lhs));
    printf("size of 2 * num_users * sizeof(element_t) = %d\n", 2 * num_users * sizeof(element_t));
    printf("DEBUG INFO: \n");
    */
    ...
    printf("\nDEBUG INFO: \n");
    printf("alpha = ");
    element_out_str(stdout, 0, alpha);
    printf("\n");
